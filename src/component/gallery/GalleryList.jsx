...
const GalleryList = () => {
  ...
  const loadImages = useCallback(async (cat) => {
    try {
      ...
      const cats = Object.entries(countMap).map(([category, imageCount]) => ({
        category,
        imageCount,
      }));
  ...
  const filtered = images.filter((img) => {
    ...
    return (
      img.title?.toLowerCase().includes(q) || img.cat?.toLowerCase().includes(q)
    );
  });
  ...
  const CatBadge = ({ cat, size = "sm" }) => {
    ...
    return (
      <span
        className={`inline-flex items-center gap-1 border rounded-full font-semibold ${size === "sm" ? "text-xs px-2 py-0.5" : "text-sm px-3 py-1"} ${a.light}`}
      >
      ...
      <div className="gl-root">
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            <div>
              <h1 className="gl-heading text-4xl font-bold text-gray-900 mb-1">
                Gallery
              </h1>
              <p className="text-sm text-gray-400">
                Manage and organise your clinic images
              </p>
            </div>
            <button
              className="add-btn"
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
            {[
              ...
              {
                label: "Active Filter",
                value: selectedCat === "all" ? "All" : selectedCat,
                accent: true,
              },
              ...
              <div key={label} className="stat-card fade-up">
                ...
                <p
                  className={`text-2xl font-bold ${accent ? "text-amber-500" : "text-gray-900"}`}
                >
                  {value}
                </p>
              </div>
                ...
                >
                  ...
                  {cat !== "all" && (
                    <span className="ml-1 opacity-50 text-xs">
                      ({categories.find((c) => c.category === cat)?.imageCount})
                    </span>
                  )}
                </button>
              ))}
                  ...
                  >
                    <div className="gl-img-wrap" style={{ height: 200 }}>
                      <img
                        ...
                        onError={(e) => {
                          e.target.src =
                            "https://placehold.co/400x200?text=Error";
                        }}
                      />
                      <div className="gl-cat-pill">
                        <span
                          className={`inline-block w-2 h-2 rounded-full mr-1.5 ${acc.bg}`}
                    ...
                    <div style={{ padding: "14px 16px" }}>
                      ...
                      <div className="flex gap-2">
                        <button
                          className="icon-btn view flex-1"
          ...
          {!loading && filtered.length > 0 && viewMode === "list" && (
            <div className="flex flex-col gap-2">
              {filtered.map((img, i) => (
                ...
                >
                  <img
                    src={img.url || "https://placehold.co/56x56?text=No+Img"}
                    alt={img.title}
                    style={{
                      width: 56,
                      height: 56,
                    ...
                    onError={(e) => {
                      e.target.src = "https://placehold.co/56x56?text=Err";
                    }}
                  />
      ...
      {deleteModal.show && (
        <div className="lightbox-bg" style={{ background: "rgba(0,0,0,0.5)" }}>
          ...
          >
            ...
            >
              <strong style={{ color: "#111" }}>"{deleteModal.title}"</strong>{" "}
